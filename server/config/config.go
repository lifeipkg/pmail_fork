...
func (c *Config) fixPath() {
	// 统一转换为系统路径分隔符
	ROOT_PATH := filepath.FromSlash(ROOT_PATH)

	// 处理 SQLite 数据库路径
	if c.DbType == DBTypeSQLite && !isAbsolutePath(c.DbDSN) {
		c.DbDSN = filepath.Join(ROOT_PATH, filepath.FromSlash(c.DbDSN))
	}

	// 处理 SSL 证书路径
	normalizePath := func(path *string) {
		if !isAbsolutePath(*path) {
			*path = filepath.Join(ROOT_PATH, filepath.FromSlash(*path))
		}
		// 确保路径格式统一化
		*path = filepath.Clean(*path)
	}

	normalizePath(&c.SSLPublicKeyPath)
	normalizePath(&c.SSLPrivateKeyPath)
}

// 跨平台判断绝对路径
func isAbsolutePath(path string) bool {
	if runtime.GOOS == "windows" {
		// Windows 下检查盘符或 UNC 路径
		return filepath.IsAbs(path) || strings.Contains(path, ":\\") || strings.HasPrefix(path, "\\\\")
	}
